...
describe('selectorAction', () => {
  ...
  it('throws if called without arguments', () => {
    expect(() => selectorAction()).to.throw(/selectorAction called with no arguments/i);
  });
  ...
  it('throws if passed a non-function action creator', () => {
    ...
    expect(() => selectorAction({ baz: true })).to.throw(actionCreatorErrorRegex);
  ...
  it('throws if passed a non-function selector', () => {
    ...
    expect(() => selectorAction('foo', actionCreator)).to.throw(selectorErrorRegex);
    expect(() => selectorAction(42, actionCreator)).to.throw(selectorErrorRegex);
    expect(() => selectorAction({ baz: true }, actionCreator)).to.throw(selectorErrorRegex);
    expect(() => selectorAction([1, 2, 3], actionCreator)).to.throw(selectorErrorRegex);
  });
  ...
  it('calls actionCreator with the results of 2 selectors and the state', () => {
    ...
    const generatedActionCreator = selectorAction(fooSelector, bazSelector, actionCreator);
    generatedActionCreator()(fakeDispatch, () => state);
    expect(actionCreator).to.have.been.calledWithExactly(state.FOO, 'baz', state);
    expect(fakeDispatch).to.have.been.calledWith(fakeAction);
  });
  ...
  it('exposes the original action creator so it can be tested directly', () => {
    ...
    const bazSelector = () => 'baz';
    const generatedActionCreator = selectorAction(fooSelector, bazSelector, actionCreator);
    expect(generatedActionCreator.originalActionCreator).to.equal(actionCreator);
  });

  it('dispatches the action directly when used as a thunk', () => {
    const activeIdSelector = state => state.activeId;
    const setActiveItemName = newName => selectorAction(activeIdSelector, activeId => ({
      type: 'SET_ITEM_NAME',
    ...
    const expectedAction = { type: 'SET_ITEM_NAME', payload: { activeId: 42, newName: 'johnny' } };
